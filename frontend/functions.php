<?php
    ...
    function getRoute($uri) {
        ...
        if (false != $id = isPage($route[0])) {
            ...
            $new_route[1] = isset($route[1]) ? $route[1] : '';
        }
    ...
    function isProduct($i) {
        ...
        $result = $db->query("SELECT sku FROM products WHERE product_url = '$i'");
...
function calcSalesTax($subtotal, $region) {
  global $db_connection, $sales_tax_regions;
...
function createEditForm($table_name, $key = array(), $hide_fields = array()) {
	...
	$result = $db_connection2->query("SELECT * FROM " . $table_name . " WHERE " . $key[0] . "=" . $key[1]);

	$form_data = $result->fetch_all();

	$stmt = $db_connection->prepare("SHOW FULL COLUMNS FROM $table_name");
	...
	while ($stmt->fetch()) {
		...
		if (!in_array($col1, $hide_fields)) {
			...
			if ($col5 != 'PRI' && $col5 != 'MUL' && $col7 != 'auto_increment') {
				...
				$result = $db_connection2->query("SELECT REFERENCED_TABLE_NAME, REFERENCED_COLUMN_NAME
				...
				$result = $db_connection2->query("SHOW FULL COLUMNS FROM suppliers
												WHERE Comment = 'DISPLAY_ON_SELECT'");
				$display_field = $result->fetch_object();

				$stmt2= $db_connection2->prepare("SELECT " . $ref_data->REFERENCED_COLUMN_NAME .", " . $display_field->Field .
				...
				$stmt2->close();
                        ...
                        $result = $db_connection2->query("SHOW COLUMNS FROM ".  $fk_table);
                        $cols = $result->fetch_object();

                        $stmt2 = $db_connection2->prepare("SELECT store_id, store_name, city FROM " . $fk_table);
...
function dbConnect() {
	...
	$db_connection = new mysqli($config['dbhost'], $config['dbuser'], $config['dbpwd'], $config['db']);

/*	if (!$db_connection->real_connect('localhost', 'hotworkorders', '1stank-mofo', 'hwo_sample')) {
